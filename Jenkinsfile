...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]){
        stage('Deploy Code'){
            ...
            if (isUnix()) {
                rmsg = sh returnStdout: true, script: "${toolbelt} project deploy start --metadata ApexClass:Hello"
            } else {
                rmsg = bat returnStdout: true, script: "\"${toolbelt}\" project deploy start --metadata ApexClass:Hello"
            }
